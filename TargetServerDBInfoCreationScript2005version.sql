...
BEGIN
  SET NOCOUNT ON
  --DATEADD(day, DATEDIFF(day, 0, @CDATE), 0)
	IF EXISTS(SELECT  SnapshotDate FROM dbo.DBinfo AS DB WHERE DATEADD(day, DATEDIFF(day, 0, SnapshotDate), 0) =  DATEADD(day, DATEDIFF(day, 0, GETDATE()), 0) )
		DELETE FROM dbo.DBinfo WHERE DATEADD(day, DATEDIFF(day, 0, SnapshotDate), 0) =  DATEADD(day, DATEDIFF(day, 0, GETDATE()), 0)
