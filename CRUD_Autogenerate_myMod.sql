...
--		suffixes:
--		1) List all records in the table (Prefix of 'List')
--		2) Get a specific record from the table (Prefix of 'Select')
--		3) UPDATE or INSERT (UPSERT) - (Prefix of 'Edit')
--		4) DELETE a specified row - (Prefix of 'Delete')
...
-- Modified by Dave Bennett 2/25/2016
-- #########################################################
...
WHILE @@FETCH_STATUS = 0 BEGIN
	...
	IF @TableName <> @CurrentTable BEGIN
		...
		IF @GenerateProcsFor = '' OR @GenerateProcsFor = @CurrentTable BEGIN
			...
			-- _lst
			SET @LIST = 'CREATE PROC [dbo].[List' + @ObjectName + ']' + Char(13)
			...
			-- _sel
			SET @SELECT = 'CREATE PROC [dbo].[Select' + @ObjectName + ']' + Char(13)
			...
			-- _ups
			SET @UPSERT = 'CREATE PROC [dbo].[Edit' + @ObjectName + ']' + Char(13)
			...
			-- _del
			SET @DELETE = 'CREATE PROC [dbo].[Delete' + @ObjectName + ']' + Char(13)
